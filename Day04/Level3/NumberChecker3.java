...
class NumberChecker3 {
    ...
    static boolean isPalindrome(int[] digitArr) {
        ...
        if (digitArr.length != reversedArr.length) {
            return false;
        }
        for (int i = 0; i < digitArr.length; i++) {
            if (digitArr[i] != reversedArr[i]) {
                return false;
            }
        }
        return true;
    ...
    static boolean isDuckNumber(int[] digitArr) {
        boolean check = false;
    ...
    public static void main(String[] args) {
        ...
        int size = NumberChecker3.countOfDigit(number);

        // Convert the number into an array of digits
        int[] digitArr = NumberChecker3.digitsIntoArr(number, size);

        // Reverse the digits array
        int[] reversedArr = NumberChecker3.reverseArray(digitArr);

        // Check if the number is a palindrome
        boolean checkPalindrome = NumberChecker3.isPalindrome(digitArr);

        // Check if the number is a Duck Number
        boolean checkDuckNumber = NumberChecker3.isDuckNumber(digitArr);
